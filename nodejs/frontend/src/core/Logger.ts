...
export class Logger {
    ...
    const measureFrameRate = () => {
      ...
      if (currentTime - lastTime >= 1000) {
        ...
        if (fps < this.config.performance.fpsThreshold) {
          this.warn('Performance', 'startFrameRateMonitoring', `Low FPS detected: ${fps}`, {
            fps,
            threshold: this.config.performance.fpsThreshold,
          });
        }
      ...
      setInterval(() => {
        ...
        if (memoryUsage > this.config.performance.memoryThreshold) {
          this.warn(
            ...
            { memoryUsage, threshold: this.config.performance.memoryThreshold }
          );
        }
  ...
  private createLogEntry(
    ...
    const entry: LogEntry = {
      ...
      performance: this.isObjectPerformanceEnabled(objectName)
  ...
  private outputToConsole(entry: LogEntry): void {
    ...
    const formattedMessage = this.formatMessage(
      entry.objectName,
      LogLevel[entry.level as keyof typeof LogLevel],
      entry.message,
      entry.data,
      entry.methodName
    );
...
export const logGameEvent = (eventName: string, eventData: any, playerId?: string) =>
  logger.logGameEvent(eventName, eventData, playerId);
