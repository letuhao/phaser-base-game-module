...
export function testEnhancedResponsiveConfigLoader() {
  ...
  if (config1) {
    ...
    testWidths.forEach(width => {
      ...
      console.log(`   Width ${width}px: breakpoint="${currentBreakpoint}", header height=${headerLayout?.height}`)
    })
    
    // Get all layouts for header
    const allHeaderLayouts = ResponsiveConfigLoader.getObjectLayoutsAcrossBreakpoints(config1, 'header')
  ...
  if (config2) {
    ...
    testWidths.forEach(width => {
      ...
      console.log(`   Width ${width}px: breakpoint="${currentBreakpoint}", scaleMode=${canvasLayout?.scaleMode}`)
    })
  }
  ...
  if (config3) {
    ...
    testWidths.forEach(width => {
      ...
      console.log(`   Width ${width}px: breakpoint="${currentBreakpoint}", position=${navLayout?.position}`)
    })
  }
...
export {
  customBreakpointConfig,
  gameBreakpointConfig,
  orientationBreakpointConfig
}
