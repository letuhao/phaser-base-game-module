 ...
 */
export const ASSET_TYPE_METADATA: Record<
  AssetType,
  {
    name: string;
    description: string;
...
export class AssetLoaderConfigLoader {
  ...
  public registerConfig(sceneName: string, config: AssetLoaderConfig): void {
    ...
    logger.info(
      'AssetLoaderConfigLoader',
      'registerConfig',
      'registerConfig',
      'Registered asset loader config for scene: ${sceneName}'
    );
  }
  ...
  public loadConfig(sceneName: string): AssetLoaderConfig | null {
    ...
    if (!config) {
      logger.warn(
        ...
        'loadConfig',
        'loadConfig',
        'No asset loader config found for scene: ${sceneName}'
      );
      return null;
    }
    ...
    logger.info(
      ...
      'loadConfig',
      'loadConfig',
      'Loaded asset loader config for scene: ${sceneName}'
    );
    return config;
  }
