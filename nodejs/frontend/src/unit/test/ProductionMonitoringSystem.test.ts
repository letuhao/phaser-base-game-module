...
describe('ProductionMonitoringSystem', () => {
  ...
  describe('Integration Scenarios', () => {
    it('should handle full monitoring lifecycle', () => {
      ...
      monitoringSystem.recordMetric('test.metric', 100, 'ms');
      monitoringSystem.recordCalculationPerformance('test-calculation', 50, 1024, {}, { id: 'test-unit', name: 'TestUnit', sizeUnit: SizeUnit.PARENT_WIDTH, dimension: Dimension.WIDTH, baseValue: SizeValue.FILL });
    ...
    it('should handle high-volume metric recording', () => {
      ...
      for (let i = 1; i < recentMetrics.length; i++) {
        expect(recentMetrics[i].timestamp.getTime()).toBeGreaterThanOrEqual(recentMetrics[i - 1].timestamp.getTime());
      }
    });
