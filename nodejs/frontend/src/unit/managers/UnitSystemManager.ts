...
export class UnitSystemManager implements IUnitSystemManager {
  ...
  // Performance and monitoring
  getPerformanceMetrics() {
    const avgTime = this.performanceMetrics.calculationTimes.length > 0
      ? this.performanceMetrics.calculationTimes.reduce((a, b) => a + b, 0) / this.performanceMetrics.calculationTimes.length
      : 0
    ...
    return {
      ...
      errorRate: this.performanceMetrics.errors / Math.max(this.performanceMetrics.totalCalculations, 1)
    }
  }
