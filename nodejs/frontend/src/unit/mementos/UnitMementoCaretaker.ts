...
export class UnitMementoCaretaker implements IUnitMementoCaretaker {
  ...
  getMemento(unitId: string, timestamp: Date): IUnitMemento | undefined {
    ...
    return unitMementos.find(memento => 
      memento.getTimestamp().getTime() === timestamp.getTime()
    )
  }
  ...
  deleteMemento(memento: IUnitMemento): boolean {
    ...
    const index = unitMementos.findIndex(m => 
      m.getTimestamp().getTime() === memento.getTimestamp().getTime()
    )
  ...
  private enforceTotalMementosLimit(): void {
    ...
    allMementos.sort((a, b) => a.memento.getTimestamp().getTime() - b.memento.getTimestamp().getTime())
