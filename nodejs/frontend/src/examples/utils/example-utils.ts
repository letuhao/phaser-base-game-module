...
export class StylePropertyBuilder {
  ...
  // Typography methods
  text(family: string, size: number, weight: 'normal' | 'bold' | 'bolder' | 'lighter' | 100 | 200 | 300 | 400 | 500 | 600 | 700 | 800 | 900 = 'normal', color: string = '#000000'): this {
...
export class StylePropertyValidator {
  static validate(properties: Partial<IStyleProperties>): {
    ...
    if (properties.width === 'fill' && properties.positionX !== 'center' && properties.positionX !== 0) {
      warnings.push('Full-width objects typically use center or left positioning')
    }
    
    // Check for reasonable values
    if (properties.fontSize && typeof properties.fontSize === 'number' && (properties.fontSize < 8 || properties.fontSize > 72)) {
      warnings.push('Font size should typically be between 8 and 72 pixels')
    }
    
    if (properties.alpha && typeof properties.alpha === 'number' && (properties.alpha < 0 || properties.alpha > 1)) {
      errors.push('Alpha value must be between 0 and 1')
    }
    
    if (properties.borderRadius && typeof properties.borderRadius === 'number' && properties.borderRadius < 0) {
