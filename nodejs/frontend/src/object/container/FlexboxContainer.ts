...
export class FlexboxContainer extends Container {
  ...
  private calculateFlexboxLayout(
    ...
    const isReverse =
      this.flexbox.direction === 'row-reverse' || this.flexbox.direction === 'column-reverse';
    ...
    flexItems.forEach(({ child, properties, size, flexBasis }) => {
      const flexGrowSpace =
        properties.flexGrow > 0 ? (properties.flexGrow / totalFlexGrow) * totalFlexGrowSpace : 0;
  ...
  private getChildSize(child: IGameObject): { width: number; height: number } {
    if (
      'size' in child &&
      child.size &&
      typeof child.size === 'object' &&
      'width' in child.size &&
      'height' in child.size
    ) {
  ...
  clone(): IGameObject {
    ...
    const cloned = new FlexboxContainer(
      this.scene,
      `${this.id}_clone`,
      this.x,
      this.y,
  ...
  initialize(): void {
    this.logger.debug('FlexboxContainer', 'initialize', 'Initializing flexbox container', {
      id: this.id,
    });
  ...
  activate(): void {
    this.setActive(true);
    this.logger.debug('FlexboxContainer', 'activate', 'Flexbox container activated', {
      id: this.id,
    });
  }
  ...
  deactivate(): void {
    ...
    this.logger.debug('FlexboxContainer', 'deactivate', 'Flexbox container deactivated', {
      id: this.id,
    });
  }
