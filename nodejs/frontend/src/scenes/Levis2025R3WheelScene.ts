...
export class Levis2025R3WheelScene extends Phaser.Scene {
  ...
  async preload(): Promise<void> {
    this.logger.info('Levis2025R3WheelScene', 'preload', 'Starting asset loading with asset system');
    ...
    try {
      ...
      this.logger.info('Levis2025R3WheelScene', 'preload', 'Assets loaded successfully by asset system');
    } catch (error) {
  ...
  create(): void {
    ...
    if (!this.assetsLoaded) {
      this.logger.warn('Levis2025R3WheelScene', 'create', 'Assets not loaded, skipping object creation');
      return;
    }
    ...
    try {
      ...
      if (this.assetManager.isAssetLoaded(backgroundKey)) {
        ...
        this.logger.warn('Levis2025R3WheelScene', 'create', 'Background asset not loaded', { backgroundKey });
      }
      ...
      this.logger.error('Levis2025R3WheelScene', 'create', 'Failed to create scene objects', { error });
    }
  }
