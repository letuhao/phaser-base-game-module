...
export interface IEnvironmentalEffect extends IEffect {
  ...
  setEnvironmentalSoundFade(enabled: boolean, duration?: number, distance?: number, falloff?: number): this;
  
  /** Set environmental sound 3D positioning */
  setEnvironmentalSound3D(enabled: boolean, position?: Phaser.Math.Vector3, velocity?: Phaser.Math.Vector3): this;
  
  /** Set environmental sound 3D rolloff */
  setEnvironmentalSound3DRolloff(rolloffFactor: number, referenceDistance: number, maxDistance: number): this;
  ...
  getEnvironmentalSound3DReverb(): { roomSize: number; damping: number; wet: number; dry: number; width: number; freeze: boolean };
